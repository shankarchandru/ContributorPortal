define([
	...
	"dojo/domReady!"
	],function(declare,Toolbar,Dialog,DropDownButton,TooltipDialog,TextBox,Button,Select,PopUp,Registry,_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin, OnDemandGrid, Selection, ReviewerResultsStore, lang, dojoColor, arrayUtil, domUtil, QueryTask, Query, ReviewerLifecycle,ReviewerResultsTask, ReviewerFilters,SimpleMarkerSymbol, SimpleLineSymbol, SimpleFillSymbol, Graphic,template) {
	...
	return declare([_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin], {
		...
		postCreate: function () {
			...
			//Create an Alert Dialog for errors
			this._messageDialog = new Dialog({
				title: "Message",  //Set Default Title and update it while using it.
				style: "width: 450px"
			});			
			 ...
			 if (this._rowsSelected.length > 0){
			   ...
			   //get lifecycle information from the ReviewerLifecycle helper class
				...
				if (lifecycleInfo !== null){
					...
					this._messageDialog.set("title", "Update Status - Error");
					this._messageDialog.set("content", "<para>The selected records cannot be updated <br/> <br/> No common lifecycle status to update</para>");
				...
				PopUp.close(toolTipDialog);
				this._messageDialog.set("title", "Update Status");
				this._messageDialog.set("content", "Please select records to update");
				this._messageDialog.show();
		...
		updateLifeCycleStatus:function (){
			...
			deferred.then(lang.hitch(this,function(response){
				...
				var query = {};
				this._resultsGrid.setQuery(this._resultsGrid.lastQuery);				/*arrayUtil.forEach(response.featureEditResults,lang.hitch(this,function(featureEditResult){
				...
				this._messageDialog.set("title", "Update Status - Error");
				this._messageDialog.set("content", error.message);
				this._messageDialog.show();				
				//alert(error.message);
			}));
		},
